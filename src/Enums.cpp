...
std::string AlgorithmType::enum2Str(AlgorithmType::enum_type const& e) const
{
    ...
    if (init)
    {
        ...
        ENUM2STR(mEnum2Str, ILP_GUROBI);
        ENUM2STR(mEnum2Str, ILP_CBC);   
        ENUM2STR(mEnum2Str, ILP_UPDATED_GUROBI);
        ENUM2STR(mEnum2Str, LP_GUROBI);
        ENUM2STR(mEnum2Str, SDP_CSDP);
        ENUM2STR(mEnum2Str, MIS_GUROBI);
        ENUM2STR(mEnum2Str, DANCING_LINK);
        ENUM2STR(mEnum2Str, DANCING_LINK_OPT);
        ENUM2STR(mEnum2Str, SJTU);
        ENUM2STR(mEnum2Str, OPB);
        init = false;
    }
...
AlgorithmType::enum_type AlgorithmType::str2Enum(std::string const& s) const
{
    ...
    if (init)
    {
        ...
        STR2ENUM(mStr2Enum, ILP_GUROBI);
        STR2ENUM(mStr2Enum, ILP_CBC);
        STR2ENUM(mStr2Enum, ILP_UPDATED_GUROBI);
        STR2ENUM(mStr2Enum, LP_GUROBI);
        STR2ENUM(mStr2Enum, SDP_CSDP);
        STR2ENUM(mStr2Enum, MIS_GUROBI);
        STR2ENUM(mStr2Enum, DANCING_LINK);
        STR2ENUM(mStr2Enum, DANCING_LINK_OPT);
        STR2ENUM(mStr2Enum, SJTU);
        STR2ENUM(mStr2Enum, OPB);
        init = false;
    }
